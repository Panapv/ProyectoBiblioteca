...
function MenuScreenBETA() {
    const [gameSetting, setGameSetting] = useState(null);

    const handleGameSetting = (value) => {
    ...
    if (gameSetting === null) {
        return (
            <>
                ...
                <div>
                    <button onClick={() => handleGameSetting('infinitive')}>Infinitivos</button>
                    <button onClick={() => handleGameSetting('present-past')}>Presente & Pasado</button>
                    <button onClick={() => handleGameSetting('mix')}>Turbo Remix Edition: Electric Boogaloo</button>
                </div>
            </>
        );
    } else if (typeof gameSetting === 'string') {
        return (
            <GameMenu gameType={gameSetting} arrVerbos={getVerbos()} />
        );
    } else {
        handleGameSetting(null);
    }
}

function GameHeader({ onChangeMinigame, onBack }) {
    return (
        <div>
...
function GameScreen({ form, arrVerbos, onBack }) {
    ...
    return (
        <>
            <GameHeader onChangeMinigame={handleMinigame} onBack={onBack} />
            {(minigame == 1) ? <Game01 arrVerbos={arrVerbos} toGuess={formVerbs}/> : <Game02 arrVerbos={arrVerbos} toGuess={formVerbs}/>}
        </>
    );
}
    ...
    return (
        <>
            {!selectedForm ? (
                ...
                <GameScreen form={formSetting} arrVerbos={getVerbos()} onBack={handleBackForm} />
            )}
        </>
    );
}